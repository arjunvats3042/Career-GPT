...
export const AnimatedTestimonials = ({testimonials, autoplay = false}) => {
  ...
  // Autoplay effect with proper dependencies
  useEffect(() => {
    if (autoplay && isMounted) {
      const interval = setInterval(handleNext, 5000);
      return () => clearInterval(interval);
    }
  }, [autoplay, isMounted, testimonials.length]); // Include all dependencies
  ...
  return (
    <div className="max-w-sm md:max-w-4xl mx-auto antialiased font-sans px-4 md:px-8 lg:px-12 py-20">
      <div className="relative grid grid-cols-1 md:grid-cols-2 gap-20">
          ...
          <div className="relative h-80 w-full">
            <AnimatePresence>
              {testimonials.map((testimonial, index) => (
                <motion.div
                  key={`${index}-${testimonial.src}`} // Unique key combining index and src
                  initial={{
                    opacity: 0,
                    scale: 0.9,
                    z: -100,
        ...
        <div className="flex justify-between flex-col py-4">
          <motion.div
            key={`${active}-${testimonials[active].name}`} // More specific key
          ...
          >
            ...
            <motion.p className="text-lg text-gray-500 mt-8 dark:text-neutral-300">
              {testimonials[active].quote.split(" ").map((word, wordIndex) => (
                <motion.span
                  key={`${active}-${wordIndex}`} // Unique key per testimonial and word
